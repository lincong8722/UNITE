...
class CognitionTargetPlanner(Planner):
    def __init__(self, verbose=False, workdir=None, logdir=None):
        ...
        self.dorsal_heatmap_percentile = 95
    ...
    def __compute_candi_mask(self, app_data_path, subj):
        ...
        # self.lh_ba6_lable = lh_ba6_label
        # lh_ba6_mask = np.zeros(len(self.lh_anat_labels))
        # lh_ba6_mask[self.lh_ba6_lable] = 1
        # if self.verbose:
        #     lh_pial_mesh.point_data['label'] = lh_ba6_mask.copy()
        #     lh_pial_mesh.save(os.path.join(self.logdir, 'lh_ba6_mask.vtk'))

        # # rh BA6 label constraint
        # rh_ba6_label_file = os.path.join(os.environ.get("SUBJECTS_DIR"), subj, 'label', 'rh.BA6_exvivo.label')
        # rh_ba6_label = nib.freesurfer.read_label(rh_ba6_label_file)
        # self.rh_ba6_lable = rh_ba6_label
        # rh_ba6_mask = np.zeros(len(self.rh_anat_labels))
        # rh_ba6_mask[self.rh_ba6_lable] = 1
        # if self.verbose:
