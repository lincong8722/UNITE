...
import zipfile
import pandas as pd
from TargetAutoPlaning_Core import AphasiaTargetPlanner
    ...
    for subject in subject_list:
        ...
        if run_list[0] == 'func':
            ...
            if not use_group:
                ...
                ## 进行213parcel的计算
                parc213_DeepPrep(subject, data_path, reconall_dir, os.path.join(output_path_parcellation18, subject, run, 'Parcellation213'), run)
                ## 将分区结果复制到work_dir_run
                cmd = f'cp {os.path.join(output_path, subject, run, "Parcellation213/Parc213/WB_lh", subject, f"lh.{subject}_IndiCluster108_fs6.mgh")} {work_dir_run}/lh_parc213_fs6_surf.mgh'
                os.system(cmd)
                cmd = f'cp {os.path.join(output_path, subject, run, "Parcellation213/Parc213/WB_rh", subject, f"rh.{subject}_IndiCluster108_fs6.mgh")} {work_dir_run}/rh_parc213_fs6_surf.mgh'
                os.system(cmd)

            ## 进行计算
            planner = AphasiaTargetPlanner(verbose=False, workdir=work_dir_run)
